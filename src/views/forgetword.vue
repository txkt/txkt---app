<template>
	<div id="forgetword">
		...
		<ul class="f_ul">
			...
			<span class="f_ul_span">请填写用户名/手机号</span>
			<ul class="f_ul_ul">
			...
			<span class="f_ul_span">请输入正确的手机号格式</span>
	...
	export default{
		...
		methods:{
			fUl_two:function(dd,cc){
				...
				let OfUlTwo = document.getElementsByClassName("f_ul_two");
				let reg =/^1(5|3|8)\d{9}$/;
				if(dd == 1){
					if(reg.test(cc) == "false"){
						ofUlSpan[dd].style.display = "block";
					}else if(cc == ""){
						ofUlSpan[dd].style.display = "block";
					}else{
						ofUlSpan[dd].style.display = "none";
					}
				}else if(dd == 0){
					if(cc == ""){
						ofUlSpan[dd].style.display = "block";
...
// 					})
// 				if( == 0){
// 					ofUlSpan[1].style.display = "block";
// 				}
			},
			f_ulSend:function(){
				let ofUlSpan = document.getElementsByClassName("f_ul_span");
